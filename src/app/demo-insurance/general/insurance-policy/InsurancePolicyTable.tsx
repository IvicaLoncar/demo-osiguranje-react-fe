...
import { InsurancePolicy, getInsurancePolicyColumnDefinitions } from './InsurancePolicy.model';
...
export function InsurancePolicyTable() 
{

  const dateConvert = dateConvertUtil();

  const insurancePolicies = useAppSelector(selectInsurancePolicies);
  ...
  const deleteMessage = "Brisanje sloga sa insurancePolicy id &!";
 ...
 // const [ rowsPerPage, setRowsPerPage ] = useState(20 as number);
  ...
  const [ filterFieldInsurancePolicyID, setFilterFieldInsurancePolicyID ] = useState("" as string);
  ...
  /*function getColumnDefinitions(): ColumnDefinition[]
  {
    ...
    return         [
      { position: position++, field: 'insurancePolicyID', header: 'InsurancePolicy Id', show: true, frozen: false},
      { position: position++, field: 'insurancePolicyType', header: 'InsurancePolicy type', show: true, frozen: false},
...
*/
  ...
  const getData = (pageNumber: number, rowsPerPage: number) => {
    ...
    setTimeout(function() { 
      ...
      preparedFilter = preparedFilter + (preparedFilter.trim() !== "" ? "," : "") + (filterFieldInsurancePolicyID.trim() !== "" ? "insurancePolicyID_" + filterFieldInsurancePolicyID.trim() : "");
      ...
      dispatch(fetchInsurancePolicies(restGetParams));
  ...
  const deleteRecord = () => {
    ...
    setTimeout(function() { 
      dispatch(deleteInsurancePolicy(deleteRowID));
  ...
  const columnDefinitions: ColumnDefinition[] = getInsurancePolicyColumnDefinitions();
  ...
  const rowButtonsBodyTemplate = (rowData: InsurancePolicy) => {
    return <> 
      <Button className="p-button-text p-button-primary mr-1 -mb-2 -mt-2 p-2" onClick={() => showRowViewer(rowData.insurancePolicyID !== undefined ? rowData.insurancePolicyID : -1 )}><span className="pi pi-pencil"></span></Button>
      <Button className="p-button-text p-button-danger mr-1 -mb-2 -mt-2 p-2" onClick={() => showDeleteMessage(rowData.insurancePolicyID)}><span className="pi pi-times"></span></Button>
    </>
  } 
...
*/
  ...
  function cellStyling(value: any, options: DataTableCellClassNameOptions): object | string
  {
    ...
    return "line-height-1 overflow-hidden white-space-nowrap "  // 
      + (options.column.props.field?.trim().toUpperCase() === "insurancePolicyID".trim().toUpperCase() ? "right-100" : "");
  }
              ...
              <div className='col-6 m-0 p-0'>
                <div className='grid m-0 p-0'>
                  ...
                  <div className='col-2 m-0 p-0'>
                    <InputNumber id="insurancePolicyID" placeholder="Id" className="" size={10} />
                  </div>
    ...
    */
    ...
    <div>
      ...
      <div className="flex align-content-center justify-content-center flex-wrap" >
        ...
        {showProgressSpinner &&  
          <div className="absolute z-5 flex align-content-center justify-content-center flex-wrap" style={{height: 'calc(100vh - 6rem)'}}>
            <ProgressSpinner className="flex align-items-center justify-content-center" strokeWidth="3" animationDuration="5" /> 
          </div> }

        {showMessageBox &&  
          <div className="absolute z-5 flex align-content-center justify-content-center flex-wrap " style={{height: 'calc(100vh - 6rem)'}}>
              <MessageBox title="Brisanje" message={deleteMessage.replace("&", deleteRowID.toString())}  confirmed={deleteRecord} canceled={cancelDelete} ></MessageBox> 
          </div>}
      </div>
        ...
        <div className='grid'>
          ...
          <div className='col-12 m-0 p-0 ' style={{backgroundColor: 'var(--surface-c)'}}>
            <div className='flex flex-row flex-wrap m-0 p-1'>
              <Button className="p-button p-button-primary ml-1 flex align-items-center justify-content-center" icon="pi pi-plus" onClick={() => showRowViewer(-1)} />
              <Button className="p-button p-button-primary ml-3 flex align-items-center justify-content-center" icon="pi pi-search" onClick={() => refreshData()} />
            </div>
          </div>

          <div className='col-12 p-1'>
            <div className='m-0 p-0 ' style={{height: 'calc(100vh - 14rem)'}}>
            ...
            <DataTable 
               ...
               className="p-datatable m-0 p-0 overflow-hidden white-space-nowrap" 

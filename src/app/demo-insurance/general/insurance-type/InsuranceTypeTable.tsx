...
import { InsuranceType, getInsuranceTypeColumnDefinitions } from './InsuranceType.model';
...
export function InsuranceTypeTable() 
{

  const dateConvert = dateConvertUtil();

  const insuranceTypes = useAppSelector(selectInsuranceTypes);
  ...
  const deleteMessage = "Brisanje sloga sa insuranceType id &!";
 ...
 // const [ rowsPerPage, setRowsPerPage ] = useState(20 as number);
  ...
  const [ filterFieldInsuranceTypeID, setFilterFieldInsuranceTypeID ] = useState("" as string);
  ...
  /*function getColumnDefinitions(): ColumnDefinition[]
  {
    ...
    return         [
      { position: position++, field: 'insuranceTypeID', header: 'InsuranceType Id', show: true, frozen: false},
      { position: position++, field: 'insuranceTypeType', header: 'InsuranceType type', show: true, frozen: false},
...
*/
  ...
  const getData = (pageNumber: number, rowsPerPage: number) => {
    ...
    setTimeout(function() { 
      ...
      preparedFilter = preparedFilter + (preparedFilter.trim() !== "" ? "," : "") + (filterFieldInsuranceTypeID.trim() !== "" ? "insuranceTypeID_" + filterFieldInsuranceTypeID.trim() : "");
  ...
  const deleteRecord = () => {
    ...
    setTimeout(function() { 
      dispatch(deleteInsuranceType(deleteRowID));
  ...
  const columnDefinitions: ColumnDefinition[] = getInsuranceTypeColumnDefinitions();
  ...
  const rowButtonsBodyTemplate = (rowData: InsuranceType) => {
    return <> 
      <Button className="p-button-text p-button-primary mr-1 -mb-2 -mt-2 p-2" onClick={() => showRowViewer(rowData.insuranceTypeID !== undefined ? rowData.insuranceTypeID : -1 )}><span className="pi pi-pencil"></span></Button>
      <Button className="p-button-text p-button-danger mr-1 -mb-2 -mt-2 p-2" onClick={() => showDeleteMessage(rowData.insuranceTypeID)}><span className="pi pi-times"></span></Button>
    </>
  } 
...
*/
  ...
  function cellStyling(value: any, options: DataTableCellClassNameOptions): object | string
  {
    ...
    return "line-height-1 overflow-hidden white-space-nowrap "  // 
      + (options.column.props.field?.trim().toUpperCase() === "insuranceTypeID".trim().toUpperCase() ? "right-100" : "");
  }
              ...
              <div className='col-6 m-0 p-0'>
                <div className='grid m-0 p-0'>
                  ...
                  <div className='col-2 m-0 p-0'>
                    <InputNumber id="insuranceTypeID" placeholder="Id" className="" size={10} />
                  </div>
